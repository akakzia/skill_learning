...
for i in range(nb_seeds):
    for model in models:
        job_file = os.path.join(job_directory, "test_{}%.slurm".format(model))

        with open(job_file, 'w') as fh:
            fh.writelines("#!/bin/bash\n")
            fh.writelines("#SBATCH --account=kcr@v100\n")
            fh.writelines("#SBATCH --job-name=test_{}\n".format(model))
            fh.writelines("#SBATCH --qos=qos_gpu-t3\n")
            fh.writelines("#SBATCH --output=test_{}%_%j.out\n".format(model))
            fh.writelines("#SBATCH --error=test_{}%_%j.out\n".format(model))
            ...
            fh.writelines("srun python -u -B train.py --algo 'semantic' --n-blocks 5 --n-epochs 1000 --n-cycles 50 --n-batches 30 --architecture {} --save-dir 'test_{}/' 2>&1 ".format(model, model))
